...
CREATE TABLE pokemon (
                         ...
                         name varchar(30),
                         speed int(255),
                         special_defence int(255),
                         special_attack int(255),
                         defence int(255),
                         attack int(255),
                         hp int(255),
...
SELECT distinct primary_type FROM pokemon;

-- What are the average defence for all pokemon?
SELECT AVG(defence) FROM pokemon;

-- What are the average hp for (primary) grass types?
SELECT AVG(hp) as AvgHP_grass_types FROM pokemon WHERE primary_type='grass';

-- How many pokemon exists pr. primary type?
SELECT  primary_type, COUNT(pokedex_number) FROM pokemon GROUP BY primary_type;
-- også: SELECT COUNT(*) FROM pokemon WHERE (primary_type='fire') AND hp > (SELECT AVG(hp) FROM pokemon);
-- får kun det rigtige resultat hvis parentes om primary-type=fire

-- How many fire pokemon has higher hp than the average pokemon?
SELECT primary_type, COUNT(primary_type) FROM pokemon WHERE primary_type='fire' AND hp > (SELECT AVG(hp) FROM pokemon);

-- What primary type are the fastest?
SELECT primary_type, AVG(speed) FROM pokemon GROUP BY primary_type ORDER BY AVG(speed) DESC limit 1;

-- aggregate functions (avg(), sum() osv), skal have order by (??)
-- aggregate functions må ikke være i WHERE så brug subselect
